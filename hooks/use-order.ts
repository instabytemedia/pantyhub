...
import type { Order, OrderCreate, OrderUpdate } from "@/lib/schemas/order";
...
interface UseOrdersResult {
  ...
  create: (input: OrderCreate) => Promise<Order | null>;
  update: (id: string, input: OrderUpdate) => Promise<Order | null>;
...
export function useOrders(options: UseOrdersOptions = {}): UseOrdersResult {
  ...
  const create = useCallback(async (input: OrderCreate): Promise<Order | null> => {
    try {
  ...
  const update = useCallback(async (id: string, input: OrderUpdate): Promise<Order | null> => {
    try {
      ...
      if (!res.ok) {
        const body = await res.json();
        throw new Error(body.error?.message || "Failed to update order");
      }
      const json = await res.json();
      await fetchData();
      return json.data as Order;
  ...
  const remove = useCallback(async (id: string): Promise<boolean> => {
    try {
      ...
      if (!res.ok) {
        const body = await res.json();
        throw new Error(body.error?.message || "Failed to delete order");
      }
      await fetchData();
      return true;
    } catch (e) {
